...
Point estimate for population mean would be the statistic mean: $\bar{x}$
...
Less likely to capture the true value using a single point estimate. More likely to contain the true value using an interval. Trade-off between accuracy and precision.
...
I would expect around 5 students to be left handed. I wouldn't be surprised in 3 were left handed, but I would be surprised if 40 were left handed because it would be very different than the generalized sample.
...
```{r data_manhattan}
manhattan <- read_csv("~/R/morgan-osmun-lecture-13/manhattan.csv")
```
...
We are 95% confident that the mean will lie between the two intervals in repeated samples.
...
Interpretation: We are 95% confident that the median price of 1-bedroom apartments in Manhattan is between 2,162 and 2,875 dollars.
...
We are 95% confidence that the true proportion of mice who died while not on the treatment is between 69% and 79.3%.
...
```{r practice_2_ci}
ci_mice_died <- get_ci(boot_prop_lived, level = .90)
ci_mice_died
```
...
We can predict with 90% confidence that the true proportion of mice who lived while on the treatment is between 55.2% and 65.1%
